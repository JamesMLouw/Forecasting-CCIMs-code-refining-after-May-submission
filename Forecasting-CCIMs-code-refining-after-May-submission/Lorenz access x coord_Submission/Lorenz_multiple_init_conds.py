...
plt.plot(np.log(diff[:400]), lw = 0.7)
#%%
...
plot_with_error_bound = True, log_scale = True, time_cut_off=10, N_transient=180,
...
#%% plot prediction area for 9 trajectories

def prediction_error_plot_panel(z_errors, h, max_lyapunov_exp, plot_with_error_bound = True,
                          log_scale = True, time_cut_off = None, N_transient = 1, n_rows = 3, n_cols = 3,
                          title = None, include_title = False):
    # plots the error in prediction over time
        # z_errors - errors to plot, should have at least panel_width * panel_height trajectories in it
        ...
        # nrows, ncols - number of graphs to plot wide and high
    ...
    error_plot, error_ax = plt.subplots(n_rows,n_cols) #figsize=(20,5)
    for i in range(n_rows):
        for j in range(n_cols):
            error_ax[i,j].plot(Time, z_errors[n_rows*i+j][:T_bar], label = 'Actual errors')
            error_ax[i,j].set_xlim(0, (time_cut_off - h) * max_lyapunov_exp)
            error_ax[i,j].set_ylim(z_errors[n_rows*i+j][1])
                
            if include_title:
                error_ax[i,j].set_title(title)
                """
                if N_traj != None:    
                    if access == None:
            ...
            error_ax[i,j].set_xlabel('Lyapunov times')
            if log_scale == True:
                error_ax[i,j].set_ylabel('log prediction errors')
            else:
                error_ax[i,j].set_ylabel('prediction error')
                
            if plot_with_error_bound == True:
                # l1 = max_lyapunov_exp 
                
                if log_scale == True:
                    z_error_bounds = max(z_errors[n_rows*i+j][:N_transient]) + Time #np.array([t*l1 for t in Time]) # time scale is in Lyapunov times already
                    error_ax[i,j].set_ylim([min(z_errors[i]) - 5, max(z_errors[i]) + 5])
                if log_scale == False:
                    z_error_bounds = max(z_errors[n_rows*i+j][:N_transient]) * np.array([np.exp(t*l1) for t in Time])
                    error_ax[i,j].set_ylim([0, 2*max(z_errors)])
                    
                error_ax[i,j].plot(Time, z_error_bounds, label = 'Predicted error bound')
...
title = 'system: Lorenz, access: '+ access + ', trajectories: '+ str(1) + ', type init conds: ' + type_init_conds + ', sd: ' + str(sd) + ', training: ' + training_type
z_errors = [esn_help.average_error_traj([predictions[i]]) for i in range(4)] #[random.choice(predictions)])
prediction_error_plot_panel(z_errors, h, lorenz_esn_top_le,
plot_with_error_bound = True, log_scale = True, time_cut_off=10, N_transient=180, n_rows = 2, n_cols = 2,
title = title, include_title = True)
